/**
 * Custom Editor.js Interactive Audio Block Tool
 * Upload or paste an audio URL, then add timed chapters.
...
interface InteractiveAudioToolData {
  file?: { url: string };
  url?: string;
  caption?: string;
...
interface InteractiveAudioToolConfig {
  endpoints?: { byFile?: string };
  additionalRequestHeaders?: Record<string, string>;
  field?: string;
}

export default class InteractiveAudioTool {
  private data: {
    file?: { url: string };
    url?: string;
    caption: string;
  ...
  private config: InteractiveAudioToolConfig;
  private wrapper: HTMLElement | null = null;

  static get toolbox() {
    return {
      title: "Interactive Audio",
      icon: '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/><line x1="18" y1="5" x2="18" y2="9"/></svg>',
    };
  }
  ...
  constructor({
    ...
    data?: InteractiveAudioToolData;
    config?: InteractiveAudioToolConfig;
    api: unknown;
  }) {
    this.data = {
      file: data?.file,
  ...
  private _renderUploadUI(): void {
    ...
    container.innerHTML = `
      ...
      <div style="font-size:13px;font-weight:600;color:#374151;">Interactive Audio</div>
    `;
  ...
  private _renderAudioView(audioUrl: string): void {
    if (!this.wrapper) return;
    this.wrapper.innerHTML = "";
    ...
    badge.textContent = "Interactive Audio";
    ...
    captionInput.style.cssText =
      "width:100%;padding:6px 8px;border:none;border-bottom:1px solid #e2e8f0;font-size:13px;color:#64748b;outline:none;background:transparent;box-sizing:border-box;margin-bottom:12px;";
  ...
  private _showChapterForm(existing?: MediaChapter): void {
    ...
    saveBtn.addEventListener("click", () => {
      ...
      if (!vals.label.trim()) {
        alert("Please enter a chapter label.");
        return;
      }
      if (existing) {
        const idx = this.data.chapters.findIndex((c) => c.id === existing.id);
        if (idx !== -1) {
          this.data.chapters[idx] = { id: existing.id, time, label: vals.label, modal_title: vals.modal_title, modal_content: vals.modal_content };
        }
      } else {
  ...
  private async _uploadFile(file: File): Promise<void> {
    ...
    if (this.wrapper) this.wrapper.innerHTML = '<div style="text-align:center;padding:40px;color:#94a3b8;">Uploading audio…</div>';
    try {
